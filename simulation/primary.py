...
def byz_primary(byz_status, r_signing_key, to_curr_replica, queues, client_name, r_name, m_queue, curr_transaction, curr_view, p, visible_log, frontend_log):
    if byz_status == "no_response":
        ...
        for q_name, q in queues.items():
            if q_name != client_name and q_name != r_name:
                ...
                prep_msg = generate_preprepare_msg(r_name, q_name, manipulated_signed, curr_view, p, r_signing_key)
...
def send_preprepare(to_curr_replica, r_signing_key, queues, client_name, r_name, m_queue, curr_transaction, curr_view, p, byz_status, visible_log, frontend_log, primary_signing_key):
    if byz_status:
        ...
        for q_name, q in queues.items():
            if q_name != client_name and q_name != r_name:
                prep_msg = generate_preprepare_msg(r_name, q_name, curr_transaction, curr_view, p, primary_signing_key)
                clean_prep_msg = copy.deepcopy(prep_msg)
                clean_prep_msg["Transaction"] = str(clean_prep_msg["Transaction"].message)
                frontend_log.append(clean_prep_msg)
                q["to_machine"].put([prep_msg])
